...
The Rubin Science Platform automatically configures log output to appear in RSP notebooks via an ipython startup file (``~/.ipython/profile_default/startup/20-logging.py``) that is automatically copied into place if no such file exists.
...
To further configure which logs will appear, set the log level for the ``"lsst"`` namespace as :ref:`described at the end of the python section above <basic-python-logging>`:
